...
class SearchRequest(BaseModel):
    ...
    query: str = Field(..., min_length=1, description="Original user query (e.g., 'I want running shoes')")
    answers: Dict[str, str] = Field(..., description="User's answers to questions with question IDs as keys (e.g., {'q1': 'Casual', 'q2': '$50'})")
    questions: List[Dict[str, Any]] = Field(..., description="Full question objects with id, text, and answers (e.g., [{'id': 'q1', 'text': 'What's your preferred style?', 'answers': [...]}])")
    user_id: Optional[str] = Field(default=None, description="Optional user ID for personalized queries")
...
class SearchResult(BaseModel):
    ...
    relevance: Optional[float] = Field(default=1.0, ge=0.0, le=1.0, description="Relevance score (0.0-1.0)")
    why_matches: Optional[str] = Field(default=None, description="Explanation of why the product matches user preferences")
    additional_info: Optional[str] = Field(default=None, description="Additional metadata or notes")
    highlights: Optional[List[str]] = Field(default=None, description="Bullet-point highlights for quick display")
...
class SearchResponse(BaseModel):
    ...
    results: Optional[List[SearchResult]] = Field(default=None, description="List of search results")
